...
def _trova_comune_import(localita, provincia=None):
    if not localita:
        return None
    qs = Comune.objects.filter(name__iexact=str(localita).strip())
    if provincia:
        qs = qs.filter(provincia__iexact=str(provincia).strip())
    return qs.first()
...
def _importa_offerte_commesse(file_obj):
    ...
    for row in records:
        ...
        comune = _trova_comune_import(localita, provincia)
